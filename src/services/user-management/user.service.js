...
const transporter = nodemailer.createTransport({
    ...
    const createUser=async function(req){
        try{
           ...
           // console.log("password generate in user service ",password)
            ...
            status=1;
            let query=`use [z_scope] Insert into adminmaster_gen (vcFirstName,vcLastName,designation,roleId,vcEmail,vcMobile,vcPassword,btstatus,addedby,business_vertical,vcUserName) 
            OUTPUT INSERTED.bintID_pk values (@firstName,@lastName,@designationId,@roleId,@email,@mobileNo,@password,@status,@addedBy,@businessVertical,'SCS$2025')`;
   ...
   const editUser=async function(req) {
        try {
            ...
            if(status=='Active'){
                query2 = `
                    INSERT INTO [UAD_BI_LEAD_TIME].[dbo].[Audit_log](userID, status, IP, operation)
                    VALUES(@updatedBy, 1, @publicIp, 'user edit');
                `;
            }
            else{
                query2 = ` use [UAD_BI_LEAD_TIME]
                INSERT INTO [UAD_BI_LEAD_TIME].[dbo].[Audit_log](userID, status, IP, operation)
                VALUES(@updatedBy, 0, @publicIp, 'user edit');
            `;
            }
   ...
   const requestNewMail=async function (req) {
    ...
    const getUserInfo=async function(req){
        ...
        try{
            ...
            const query = `use z_scope SELECT bintId_pk as userId from adminmaster_gen
